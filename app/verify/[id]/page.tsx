...
export default function VerifySurat({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchSurat() {
      ...
      const { data } = await supabase.from('pengajuan_surat').select('*').eq('id', params.id).single();
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-gray-500">Memverifikasi dokumen...</div>;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-red-50 p-4">
        <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-md">
          ...
          <p className="text-gray-600">Surat yang Anda verifikasi tidak terdaftar di database sistem RT 17 / RW 02.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto p-4 -mt-4">
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6 no-print">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div><span className="text-gray-500">Penerbit (Tanda Tangan):</span> <br /> <span className="font-semibold text-gray-900">Zalkini, S.Sos (Ketua RT 17/RW 02)</span></div>
          </div>
        </div>
